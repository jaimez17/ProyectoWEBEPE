class CategoriasController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @categoria.update(categoria_params)
        format.html { redirect_to @categoria, notice: 'Categoria actualizada satisfactoriamente' }
  ...
  def destroy
    @categoria.destroy
    respond_to do |format|
      format.html { redirect_to categorias_url, notice: 'Categoria eliminada satisfactoriamente' }
  ...
  private
    ...
    def categoria_params
      params.require(:categoria).permit(:name, :area_id)
    end
end
